...
WaitingJob = namedtuple(
    "WaitingJob", ["priority", "time_received", "client_addr", "job_id", "msg"]
)

RunningJob = namedtuple(
    "RunningJob", ["agent_addr", "client_addr", "msg", "time_started"]
)
EnvironmentInfo = namedtuple(
    "EnvironmentInfo", ["last_id", "created_last", "agents", "type"]
)
AgentInfo = namedtuple(
    "AgentInfo", ["name", "environments"]
)  # environments is a list of tuple (type, environment)
...
class Backend(object):
    ...
    async def handle_client_message(self, client_addr, message):
        ...
        if (
            message.__class__ != ClientHello
            and client_addr not in self._registered_clients
        ):
    ...
    async def send_environment_update_to_client(self, client_addrs):
        ...
        available_environments = {
            type: list(environments.keys())
            for type, environments in self._environments.items()
        }
    ...
    async def handle_client_new_job(self, client_addr, message: ClientNewJob):
        ...
        if message.job_id in self._waiting_jobs or message.job_id in self._job_running:
            self._logger.info(
                "Client %s asked to add a job with id %s to the queue, but it's already inside. "
                "Duplicate random id, message repeat are possible causes, "
                "and both should be inprobable at best.",
                client_addr,
                message.job_id,
            )
        ...
        self._logger.info(
            "Adding a new job %s %s to the queue", client_addr, message.job_id
        )
        job = WaitingJob(
            message.priority, time.time(), client_addr, message.job_id, message
        )
    ...
    async def handle_client_kill_job(self, client_addr, message: ClientKillJob):
        ...
        if message.job_id in self._waiting_jobs:
            ...
            await ZMQUtils.send_with_addr(
                self._client_socket,
                waiting_job.client_addr,
                BackendJobDone(
                    message.job_id,
                    ("killed", "You killed the job"),
                    0.0,
                    {},
                    {},
                    {},
        ...
        # If the job is running, transmit the info to the agent
            ...
            await ZMQUtils.send_with_addr(
                self._agent_socket,
                agent_addr,
                BackendKillJob(message.job_id, previous_state),
            )
        else:
            self._logger.warning(
                "Client %s attempted to kill unknown job %s",
                str(client_addr),
                str(message.job_id),
            )
    ...
    async def handle_client_get_queue(self, client_addr, _: ClientGetQueue):
        ...
        for job_id, content in self._job_running.items():
            ...
            jobs_running.append(
                (
                    content.msg.job_id,
                    content.client_addr == client_addr,
                    agent_friendly_name,
                    content.msg.course_id + "/" + content.msg.task_id,
                    content.msg.launcher,
                    int(content.time_started),
    ...
    async def update_queue(self):
        ...
        for agent_addr in available_agents:
            ...
            self._job_running[job_id] = RunningJob(
                agent_addr, client_addr, job_msg, time.time()
            )
            self._logger.info(
                "Sending job %s %s to agent %s", client_addr, job_id, agent_addr
            )
            await ZMQUtils.send_with_addr(
                self._agent_socket,
                agent_addr,
    ...
    async def handle_agent_hello(self, agent_addr, message: AgentHello):
        ...
        self._registered_agents[agent_addr] = AgentInfo(
            message.friendly_name,
            [
                (etype, env)
                for etype, envs in message.available_environments.items()
                for env in envs
            ],
        )
            ...
            for name, environment_info in environments.items():
                if name in env_dict:
                    ...
                    if env_dict[name].last_id == environment_info["id"]:
                        ...
                        self._logger.debug(
                            "Registering environment %s/%s for agent %s",
                            environment_type,
                            name,
                            str(agent_addr),
                        )
                        ...
                        self._logger.warning(
                            ...
                            name,
                            env_dict[name].last_id,
                            env_dict[name].created_last,
                            str(agent_addr),
                            environment_info["id"],
                        ...
                        env_dict[name] = EnvironmentInfo(
                            environment_info["id"],
                            environment_info["created"],
                            env_dict[name].agents + [agent_addr],
                            environment_type,
                        )
                else:
                    # just add it
    ...
    async def handle_agent_job_started(self, agent_addr, message: AgentJobStarted):
        ...
        await ZMQUtils.send_with_addr(
            self._client_socket,
            self._job_running[message.job_id].client_addr,
    ...
    async def handle_agent_job_done(self, agent_addr, message: AgentJobDone):
        ...
        if agent_addr in self._registered_agents:
            if message.job_id not in self._job_running:
                self._logger.warning(
                    "Job result %s from agent %s was not running",
                    message.job_id,
                    agent_addr,
                )
            else:
                self._logger.info(
                    "Job %s finished on agent %s", message.job_id, agent_addr
                )
            ...
            self._logger.warning(
                "Job result %s from non-registered agent %s", message.job_id, agent_addr
            )
    ...
    async def handle_agent_job_ssh_debug(self, agent_addr, message: AgentJobSSHDebug):
        ...
        if message.job_id not in self._job_running:
            self._logger.warning(
                "Agent %s sent ssh debug info for job %s, but it is not in the list of running jobs",
                agent_addr,
                message.job_id,
            )
        await ZMQUtils.send_with_addr(
    ...
    async def run(self):
        ...
        self._loop.call_later(
            1, create_safe_task, self._loop, self._logger, self._do_ping()
        )
        ...
        for agent_addr, agent_data in list(self._registered_agents.items()):
            ...
            try:
                ...
                if ping_count > 5:
                    ...
                    await ZMQUtils.send_with_addr(
                        self._agent_socket, agent_addr, Ping()
                    )
                    delete_agent = False
            except:
            ...
            if delete_agent:
                try:
                    ...
                    self._logger.exception(
                        "Failed to delete agent %s (%s)!", agent_addr, friendly_name
                    )
