...
"""
    ...
    def _get_simple_submission(
        users: list, time: str, courseid: str, taskid: str
    ) -> str:
...
def test_simple_prefix(get_simple_archive):
    ...
    """ Run FUT """
    cmd = BASE_CMD + "--prefix={prefix} {courseid} {archive}".format(
        courseid=courseid, archive=archive, prefix=path
    )
...
def test_simple_configuration(get_simple_archive, mock_inginious_install):
    ...
    """ Run FUT """
    cmd = BASE_CMD + "--configuration={config} {courseid} {archive}".format(
        courseid=courseid, archive=archive, config=config
    )
...
def test_simple_no_configuration(get_simple_archive, mock_inginious_install):
    ...
    """ Run FUT """
    cmd = BASE_CMD + "--configuration={config} {courseid} {archive}".format(
        courseid=courseid, archive=archive, config="non-existing-path"
    )
