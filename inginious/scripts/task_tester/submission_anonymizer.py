...
try:
    ...
    from inginious.common.base import (
        load_json_or_yaml,
        loads_json_or_yaml,
        get_json_or_yaml,
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with tarfile.open(archive, "r") as tar:
            ...
            while entry := tar.next():
                ...
                new_content = re.sub(
                    r"(%s):(.*)" % "|".join(_EMPTY_FIELDS), "", content
                )

                for field, new_value in _OTHER_FIELDS:
                    new_content = re.sub(
                        r"\'%s\':(.*)" % field,
                        "'{field}': {new_value}".format(
                ...
                new_content = "\n".join(
                    filter(
                        lambda x: not re.match(r"^\s*$", x), new_content.splitlines()
                    )
                )
                ...
                new_submission_file = os.path.join(
                    task, "test", new_submission_filename
                )
                ...
                if inginious_modules:
                    ...
                    try:
                        ...
                        with open(new_submission_file, "w") as fd:
...
def main():
    ...
    parser.add_argument(
        "courseid",
        type=str,
        help="The course ID corresponding to the submissionsto anonymize.",
    )
    ...
    parser.add_argument(
        ...
        "--configuration",
        type=str,
        help="Path towards an INGInious instance configuration file.\
    ...
    parser.add_argument(
        ...
        "--prefix",
        type=str,
        help="The path towards the tasks directory of an existing INGInious instance.\
    ...
    if config_path and (
        not os.path.exists(config_path) or not os.path.isfile(config_path)
    ):
    ...
    if config_path:
        if inginious_modules:
            ...
            if prefix:
                print("Ignoring provided prefix")
            ...
            """
            print(
                "You asked to use the INGInious modules but they are not installed. Fallback on --prefix if provided."
            )

    if not prefix:
        print(
            "No prefix available. Impossible to directly install the submissions within the course directory. Fallback to\
            current directory."
        )
        prefix = os.getcwd()
