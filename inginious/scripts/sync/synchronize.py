...
def main():
    ...
    try:
        config = json.load(
            open(os.environ.get("INGINIOUS_SYNC_CONFIG", "synchronize.json"), "r")
        )
    ...
    for repo in config["repos"]:
        print(
            "\x1b[33;1m-> Synchronizing course repository : "
            + repo["course"]
            + " \033[0m"
        )
        ...
        try:
            ...
            print(
                "\x1b[31;1mERROR: Failed to load keyfile, or to add it with ssh-add\033[0m"
            )
            exit(1)
        ...
        if not os.path.exists(maindir + "/" + repo["course"]):
            ...
            if sh.git.status("-s"):
                out = sh.git.commit(
                    "-a", "-m", "Automatic synchronization from INGInious"
                )
                print_output(out.stdout.decode("utf-8"))
                print_output(out.stderr.decode("utf-8"))
