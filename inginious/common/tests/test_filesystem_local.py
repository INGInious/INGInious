...
FS = {
    filepath: {"content": content, "hash": hash(content)}
...
class TestFromSubfolder:
    ...
    def test_subfolder(self, init_subfolder):
        ...
        )  # from_subfolder must return a new FileSystemProvider object
...
class TestList:
    ...
    def test_list_file_populated_recursive(self, init_full):
        ...
        assert set(provider.list(files=True, folders=False, recursive=True)) == set(
            [file1, file2]
        )
    ...
    def test_list_folder_populated(self, init_full):
        ...
        assert set(provider.list(files=False, folders=True)) == set(
            ["%s/" % file2.split("/")[0]]
        )
    ...
    def test_list_folder_populated_recursive(self, init_full):
        ...
        provider, _, _, file2 = init_full
        assert set(provider.list(files=False, folders=True, recursive=True)) == set(
            ["%s/" % file2.split("/")[0]]
        )
    ...
    def test_list_populated(self, init_full):
        ...
        provider, _, file1, file2 = init_full
        assert set(provider.list(files=True, folders=True)) == set(
            [file1, "%s/" % file2.split("/")[0]]
        )
    ...
    def test_list_populated_recursively(self, init_full):
        ...
        assert set(provider.list(files=True, folders=True, recursive=True)) == set(
            [file1, file2, "%s/" % file2.split("/")[0]]
        )
