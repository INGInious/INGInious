...
def get_filesystems_providers():
    ...
    try:
        plugged_providers = importlib.metadata.entry_points(
            group="inginious.filesystems"
        )
    except:  # < python3.10
        plugged_providers = importlib.metadata.entry_points().get(
            "inginious.filesystems", ()
        )
...
def get_args_and_filesystem(parser):
    ...
    fs_group.add_argument(
        "--fs",
        help="(advanced users only) Name of the FSProvider to use. Using a FSProvider will add new args to be filled. "
        "Either --fs or --tasks must be filled, but not both.",
        type=str,
        choices=filesystem_providers.keys(),
    )
    parser.add_argument(
        "--fs-help",
        help="Display help to fill arguments for --fs. Only checked if --fs is filled.",
        action="store_true",
    )
    ...
    if args.fs is not None:
        ...
        for arg_name, (arg_type, arg_required, arg_desc) in fs_args_needed.items():
            fs_args_group.add_argument(
                "--fs-" + arg_name, type=arg_type, help=arg_desc, required=arg_required
            )
        if args.fs_help:
