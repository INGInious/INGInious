...
def directory_content_with_hash(directory):
    ...
    for root, _, filenames in os.walk(directory):
        for filename in filenames:
            ...
            file_stat = os.stat(p)
            with open(p, "rb") as f:
                output[os.path.relpath(p, directory)] = (
                    hash_file(f),
                    file_stat.st_mode,
                )
    return output
...
def directory_compare_from_hash(from_directory, to_directory):
    ...
    from_directory = dict(
        [
            (os.path.normpath(path), (filehash, stat))
    ...
    to_directory = dict(
        [
            (os.path.normpath(path), (filehash, stat))
            for path, (filehash, stat) in to_directory.items()
        ]
    )
...
def dict_from_prefix(prefix, dictionary):
    ...
    if len(dictionary) == 0:
        ...
        for key, val in dictionary.items():
            ...
            if ret is None:
                continue
            return_dict[ret.group(1)] = dict_from_prefix(
                "[{}]".format(ret.group(1)), dictionary
            )
        return return_dict
