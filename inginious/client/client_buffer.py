...
class ClientBuffer(object):
    ...
    def new_job(
        self, priority, course, task, inputdata, launcher_name="Unknown", debug=False
    ):
    ...
    def _callback(
        self, bjobid, result, grade, problems, tests, custom, archive, stdout, stderr
    ):
        """Callback for self._client.new_job"""
        if str(bjobid) in self._waiting_jobs:
            self._jobs_done[str(bjobid)] = (
                result,
                grade,
                problems,
                tests,
