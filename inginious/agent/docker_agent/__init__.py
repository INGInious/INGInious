...
class DockerAgent(Agent):
    def __init__(
        ...
        """
        super(DockerAgent, self).__init__(
            context, backend_addr, friendly_name, concurrency
        )

        self._runtimes = (
            {x.envtype: x for x in runtimes} if runtimes is not None else None
        )
        ...
        self._max_memory_per_slot = int(
            psutil.virtual_memory().total / concurrency / 1024 / 1024
        )
        ...
        self._external_ports = (
            set(external_ports) if external_ports is not None else set()
        )
    ...
    async def _init_clean(self):
        ...
        self._containers_running: Dict[
            str, DockerRunningJob
        ] = {}  # container_id -> info
        ...
        if self._address_host is None and len(self._containers) != 0:
            ...
            available_bare_container_images = [
                image
                for envtype_containers in self._containers.values()
        ...
        if self._address_host is None:
            ...
            self._logger.info(
                "External address for SSH remote debug is %s", self._address_host
            )
    ...
    async def _check_docker_state(self):
        ...
        async def ensure_job_closing_in_one_minute(container_id, is_student):
            ...
            d = (
                ...
                if not is_student
                else self._student_containers_running
            )
            # function to handle the processing
            f = (
                self.handle_job_closing
                if not is_student
                else self.handle_student_job_closing
            )

            if container_id in d:
                self._logger.warning(
                    "Container %s has an inconsistent state after 60 secs.",
                    container_id,
                )
        ...
        while not shutdown:
            try:
                ...
                incoherent_containers = set(self._containers_running).difference(
                    running_container_ids
                )
                incoherent_student_containers = set(
                    self._student_containers_running
                ).difference(running_container_ids)

                for container_id in incoherent_containers:
                    self._create_safe_task(
                        ensure_job_closing_in_one_minute(container_id, False)
                    )

                for container_id in incoherent_student_containers:
                    self._create_safe_task(
                        ensure_job_closing_in_one_minute(container_id, True)
                    )
                ...
                async for event in source:
                    try:
                        ...
                        if event["Type"] == "container" and event["Action"] == "die":
                            ...
                            container_id = event["Actor"]["ID"]
                            if (
                                container_id in self._containers_running
                                or container_id in self._student_containers_running
                            ):
                                self._logger.info(
                                    "Container %s did OOM, killing it", container_id
                                )
                        ...
                        self._logger.exception(
                            "Docker event parsing failed: %s", str(event)
                        )
    ...
    def __get_fd_limit(self):
        ...
        return int(fd_limits[0] / self._concurrency), int(
            fd_limits[1] / self._concurrency
        )
    ...
    def __new_job_sync(self, message: BackendNewJob, future_results):
        ...
        try:
            enable_network = (
                message.environment_parameters.get("network_grading", False)
                or self._debugger
            )
        ...
        if not course_fs.exists() or not task_fs.exists():
            self._logger.warning(
                "Task %s/%s unavailable on this agent", course_id, task_id
            )
            raise CannotCreateJobException(
        ...
        if (
            environment_type not in self._containers
            or environment_name not in self._containers[environment_type]
        ):
            self._logger.warning(
                "Task %s/%s ask for an unknown environment %s/%s",
                course_id,
                task_id,
                environment_type,
                environment_name,
        ...
        ports_needed = list(
            self._containers[environment_type][environment_name]["ports"]
        )  # copy, as we modify it later!
        ...
        ports = {}
        if (
            len(ports_needed) > 0 or self._debugger
        ):  # if ssh_debug or container debug, put time limits to 30 min.
            time_limit = 30 * 60
        ...
        try:
            ...
            self._logger.error(
                "Cannot make container temp directory! %s", str(e), exc_info=True
            )
            for p in ports:
        ...
        systemfiles_path = path_join(
            task_path, "systemfiles"
        )  # tmp_dir/id/task/systemfiles/
        ...
        if course_fs.from_subfolder("$common").from_subfolder("student").exists():
            course_fs.from_subfolder("$common").from_subfolder("student").copy_from(
                None, course_common_student_path
            )
        else:
    ...
    async def new_job(self, message: BackendNewJob):
        ...
        await self._timeout_watcher.register_container(
            out.container_id, out.time_limit, out.hard_time_limit
        )
    ...
    async def create_student_container(
        ...
        try:
            ...
            if run_as_root:
                runtime_name = {
                    k for k in self._runtimes if self._runtimes[k].run_as_root
                }.pop()
            ...
            try:
                socket_path = path_join(
                    parent_info.sockets_path, str(socket_id) + ".sock"
                )
                container_id = await self._docker.create_container_student(
                    runtime,
                    environment,
                    memory_limit,
                    parent_info.student_path,
                    socket_path,
    ...
    def buffer_has_data(self, buffer):
        ...
        return (
            len(buffer) > 4 and len(buffer) >= 4 + struct.unpack("!I", buffer[0:4])[0]
        )
            ...
            while not reader_stream.at_eof():  # CHECK HERE
                ...
                while self.buffer_has_data(buffer):
                    ...
                    if msg["type"] == "ssh_student":
                        info_student = None
                        if (
                            len(self._student_containers_running) > 0
                            and msg["container_id"] in info.student_containers
                        ):
                            info_student = self._student_containers_running[
                                msg["container_id"]
                            ]
                            await self.send_ssh_job_info(
                                info.job_id,
                                self._address_host,
                                info_student.ports[22],
                                msg["ssh_user"],
                                msg["ssh_key"],
                            )
                        else:
                            self._logger.exception(
                                "Exception: no linked student_container running."
                            )
                            self._create_safe_task(
                                self.handle_job_closing(
                                    info.container_id,
    ...
    async def _handle_student_container_outputs(
        self, student_reader_stream, grading_write_stream
    ):
                ...
                while self.buffer_has_data(buffer):
                    ...
                    try:
                        grading_write_stream.write(
                            struct.pack("!I", len(msg_encoded))
                        )  # Transfer the message without decoding it
    ...
    async def handle_running_container(self, info: DockerRunningJob, future_results):
        ...
        try:
                ...
                while self.buffer_has_data(buffer):
                    ...
                    try:
                        ...
                        if msg["type"] == "run_student":
                            ...
                            memory_limit = min(
                                msg["memory_limit"] or info.mem_limit, info.mem_limit
                            )
                            time_limit = min(
                                msg["time_limit"] or info.time_limit, info.time_limit
                            )
                            hard_time_limit = min(
                                msg["hard_time_limit"] or info.hard_time_limit,
                            ...
                            assert (
                                "/" not in socket_id
                            )  # ensure task creator do not try to break the agent :-(
                            if ssh and not (
                                info.enable_network
                                and "ssh" in info.environment_type
                                and self._ssh_allowed
                            ):
                                self._logger.error(
                            ...
                            msg["type"] == "run_student_init"
                        ):  # We use non docker-docker communication !
                            if (
                                msg["student_container_id"]
                                not in student_containers_streams
                            ):
                                student_containers_streams[
                                    msg["student_container_id"]
                                ] = await self.open_student_stream(
                                    msg["student_container_id"]
                                )
                            await self._write_to_container_stdin(
                                student_containers_streams[msg["student_container_id"]][
                                    1
                                ],
                                {
                            ...
                            "stdin",
                            "student_signal",
                        ]:  # Simply transfer to student_container
                            if (
                                msg["student_container_id"]
                                not in student_containers_streams
                            ):
                            ...
                            self._logger.info(
                                "%s %s", info_student.container_id, str(msg)
                            )
                            await self.send_ssh_job_info(
                                info.job_id,
                                self._address_host,
            ...
            self._logger.exception(
                "Exception while reading container %s output", info.container_id
            )
        ...
        if not result:
            self._logger.warning(
                "Container %s has not given any result", info.container_id
            )
    ...
    async def handle_job_closing(self, container_id, retval, manual_feedback=None):
        ...
        try:
                ...
                try:
                    ...
                    for key, item in return_value.items():
                        if not isinstance(item, accepted_types[key]):
                            ...
                            accepted_types[key] == dict and key != "custom"
                        ):  # custom can contain anything:
                    ...
                    result = "crash"
                    error_msg = (
                        "The grader did not return a readable output : {}".format(
                            str(e)
                        )
                    )
            ...
            if (
                ...
                and isinstance(manual_feedback, str)
            ):
    ...
    async def kill_job(self, message: BackendKillJob):
        ...
        try:
            if message.job_id in self._container_for_job:
                self._containers_killed[self._container_for_job[message.job_id]] = (
                    "killed"
                )
                ...
                # case.
                await self.send_job_result(
                    message.job_id, "killed", state=message.state
                )
        except asyncio.CancelledError:
            raise
        except:
        ...
        heuristic = [
            ...
            (
                ...
                lambda x, y: DockerRuntime(
                    ...
                    enables_gpu=False,
                    shared_kernel=False,
                    envtype="kata-ssh" if y else "kata",
                ),
            ),
            (
                "nvidia",
                ...
                if h_runtime in runtime:
                    for ssh_allowed in {self._ssh_allowed, False}:
                        ...
                        if v.envtype not in retval:
                            self._logger.info(
                                "Using %s as runtime with parameters %s",
                                runtime,
                                str(v),
                            )
                            retval[v.envtype] = v
