...
class MCQAgent(Agent):
    def __init__(self, context, backend_addr, friendly_name, concurrency):
        ...
        available_translations = [
            x
            for x in os.listdir(get_root_path() + "/agent/mcq_agent/i18n")
            if os.path.isdir(os.path.join(get_root_path() + "/agent/mcq_agent/i18n", x))
        ]
        self._translations.update(
            {
                lang: gettext.translation(
                    "messages", get_root_path() + "/agent/mcq_agent/i18n", [lang]
                )
    ...
    def check_answer(self, problems, task_input, language):
        ...
        for problem in problems:
            (
                ...
                state,
            ) = problem.check_answer(task_input, language)
            ...
            if problem_s_messages is not None:
                problem_messages[problem.get_id()] = (
                    ("success" if problem_is_valid else "failed"),
                    problem_s_messages,
                )
    ...
    async def new_job(self, msg: BackendNewJob):
        ...
        if not translations_fs.exists():
            translations_fs = course_fs.from_subfolder("$common").from_subfolder(
                "$i18n"
            )
        if not translations_fs.exists():
            translations_fs = (
        ...
        if translations_fs.exists() and translations_fs.exists(language + ".mo"):
            translations = {
                language: gettext.GNUTranslations(
        ...
        for problemid, problem_content in task_problems.items():
            ...
            problems.append(
                problem_class(problemid, problem_content, translations, task_fs)
            )

        result, need_emul, text, problems, error_count, mcq_error_count, state = (
            self.check_answer(problems, msg.inputdata, language)
        )

        internal_messages = {
            "_wrong_answer_multiple": _(
        ...
        for key, (p_result, messages) in problems.items():
            messages = [
                internal_messages[message] if message in internal_messages else message
        ...
        if need_emul:
            self._logger.warning(
                "Task %s/%s is not a pure MCQ but has env=MCQ",
                msg.course_id,
                msg.task_id,
            )
