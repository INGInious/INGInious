...
class MongoLTILaunchDataStorage(LaunchDataStorage):
    ...
    def __init__(self, courseid, taskid, *args, **kwargs) -> None:
        ...
        self._session_cookie_name = (
            ""  # Disables session scope mechanism in favor of query_context
        )
    ...
    def set_value(self, key: str, value, exp) -> None:
        LaunchData.objects(key=key, context=self.query_context).update(
...
class LTIGradeManager(LTIScorePublisher):
    ...
    def process(self, mongo_entry: LTIGrade, grade):
        courseid, taskid, message_launch_id = (
            mongo_entry.courseid,
            mongo_entry.taskid,
            mongo_entry.message_launch_id,
        )
        ...
        try:
            ...
            if ags.can_put_grade():
                sc = Grade()
                sc.set_score_given(grade).set_score_maximum(100.0).set_timestamp(
            ...
            self._logger.error(
                "An exception occurred while sending a grade to the LTI Platform.",
                exc_info=True,
            )
