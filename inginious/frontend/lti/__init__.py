...
class LTIScorePublisher(threading.Thread, metaclass=ABCMeta):
            ...
            while not self._stopped:
                ...
                try:
                    grade = self._user_manager.get_task_cache(
                        data["username"], data["courseid"], data["taskid"]
                    )["grade"]
                except Exception:
                    self._logger.error(
                        "An exception occurred while getting a course/LTI secret/grade.",
                        exc_info=True,
                    )
                    return False
                ...
                if self.process(data, grade):
                    data.delete()
                    self._logger.debug(
                        "Successfully sent grade to TC: %s",
                        str(data.to_mongo().to_dict()),
                    )
                    continue
    ...
    def add(self, submission):
        ...
        for username in submission["username"]:
            ...
            search.update(
                {key: submission[key] for key in self._submission_tags.keys()}
            )
            entry = self._mongo_document.objects(**search).modify(
                nb_attempt=0, upsert=True, new=False
            )
    ...
    def tag_submission(self, submission, lti_info):
        ...
        for submission_key, lti_info_key in self._submission_tags.items():
            if lti_info[lti_info_key] is None:
                self._logger.error(
                    lti_info_key
                    + " is None, but grade needs to be sent back to LTI platform! Ignoring."
                )
                return
