...
def _migrate_from_v_0_6(content):
    ...
    if "environment" in content:
        ...
        content["environment_type"] = (
            "docker" if content["environment_id"] != "mcq" else "mcq"
        )
        del content["environment"]
        content["environment_parameters"] = {
...
class Task(object):
    ...
    def __init__(self, courseid: str, taskid: str, content: dict[str, Any]):
        ...
        self._task_fs = (
            get_fs_provider().from_subfolder(courseid).from_subfolder(taskid)
        )
        ...
        if env_type_obj is None:
            raise Exception(
                _("Environment type {0} is unknown").format(self._environment_type)
            )
        # Ensure that the content of the dictionary is ok
        self._environment_parameters = env_type_obj.check_task_environment_parameters(
    ...
    def input_is_consistent(
        ...
        for problem in self._problems:
            if not problem.input_is_consistent(
                task_input, default_allowed_extension, default_max_size
            ):
                return False
        return True
    ...
    def _create_task_problem(self, problemid, problem_content):
        ...
        return get_problem_types().get(problem_content.get("type", ""))(
            problemid, problem_content, self._translations, self._task_fs
        )
    ...
    def get_context(self, language):
        ...
        return (
            ParsableText(vals[0], "rst") if len(vals) else ParsableText(context, "rst")
        )
    ...
    def get_dispenser_settings(self, fields):
        ...
        return {
            field_class.get_id(): self._data[field]
    ...
    def save(self):
        ...
        if self._new_doc:
            logging.getLogger("inginious.task").info(
                "Task %s created in the factory.", self._task_fs.prefix
            )
    ...
    def get(cls, courseid: str, taskid: str):
        ...
        task = fetch_or_cache(
            task_fs, "task.yaml", lambda: _load_task(task_fs, courseid, taskid)
        )
        ...
        i18n_paths = [
            ...
            course_fs.from_subfolder("$common")
            .from_subfolder("student")
            .from_subfolder("$i18n"),
        ]
                ...
                for f in i18n_fs.list(folders=False, files=True, recursive=False):
                    ...
                    if ext == ".mo":
                        translations[lang] = fetch_or_cache(
                            i18n_fs,
                            f,
                            lambda: gettext.GNUTranslations(i18n_fs.get_fd(f)),
                        )
                break
    ...
    def delete(self):
        ...
        logging.getLogger("inginious.task").info(
            "Task %s erased from the factory.", self._task_fs.prefix
        )
