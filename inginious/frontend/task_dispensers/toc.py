...
class TableOfContents(TaskDispenser):
    ...
    legacy_fields = {
        ...
        "stored_submissions": SubmissionStorage,
    ...
    def __init__(self, task_list_func, dispenser_data, course_id):
        ...
        parse_tasks_config(
            self._task_list_func().keys(), self.config_items, self._task_config
        )
    ...
    def get_accessibilities(self, taskids, usernames):
        ...
        return {
            username: {
                taskid: Accessibility.get_value(self._task_config.get(taskid, {}))
    ...
    def get_all_categories(self):
        ...
        return (
            set(reduce(concat, [self.get_categories(taskid) for taskid in taskids]))
            if len(taskids)
            else []
        )
    ...
    def get_course_grades(self, user_tasks, usernames):
        ...
        return {
            username: round(tasks_scores[username][0] / tasks_scores[username][1])
            if tasks_scores[username][1] > 0
            else 0
            for username in usernames
        }
    ...
    def render_edit(self, course, task_data, task_errors):
        ...
        config_fields = {
            ...
            "hidden_if_empty": SectionConfigItem(
                _("Hidden if empty"), "checkbox", False
            ),
        }
        return render_template(
            "course_admin/task_dispensers/toc.html",
            course=course,
    ...
    def render(self, course, tasks_data, tag_list, username):
        ...
        accessibilities = course.get_task_dispenser().get_accessibilities(
            self._task_list_func(), [username]
        )
        return render_template(
            "task_dispensers/toc.html",
            course=course,
    ...
    def check_dispenser_data(self, dispenser_data):
        ...
        if result:
            result, errors = check_task_config(
                result.get_tasks(), self.config_items, new_toc.get("config", {})
            )
        if result is not None and new_toc:
            new_toc["imported"] = dispenser_data.get(
                "imported", False
            ) or self._dispenser_data.get("imported", False)
    ...
    def get_ordered_tasks(self):
        ...
        return OrderedDict(
            [
                (taskid, tasks[taskid])
                for taskid in self._toc.get_tasks()
                if taskid in tasks
            ]
        )
    ...
    def import_legacy_tasks(self):
        ...
        for taskid, task in self._task_list_func().items():
            try:
                ...
                dispenser_data["config"][taskid].update(
                    task.get_dispenser_settings(self.legacy_fields)
                )
