...
from inginious.common.filesystems import (
    FileSystemProvider,
    fetch_or_cache,
    invalidate_cache,
    get_fs_provider,
...
class Course(object):
    ...
    def __init__(self, courseid, content):
        ...
        try:
            ...
            if self._registration_ac not in [None, "username", "binding", "email"]:
                raise Exception(
                    "Course has an invalid value for registration_ac: " + self.get_id()
                )
            self._registration_ac_accept = self._content.get(
            ...
            self._archive_date = (
                datetime.fromisoformat(self._content["archive_date"]).astimezone()
                if "archive_date" in self._content
                else None
            )
            ...
            task_dispenser_class = get_task_dispensers().get(
                self._content.get("task_dispenser", "toc"), TableOfContents
            )
            # Here we use a lambda to ensure we do not pass a fixed list of tasks to the task dispenser
            self._task_dispenser = task_dispenser_class(
                lambda: self.get_tasks(),
                self._content.get("dispenser_data", {}),
                self.get_id(),
            )
        except:
    ...
    def is_registration_possible(self, user_info: UserInfo):
        ...
        return (
            self.get_accessibility().is_open()
    ...
    def get_accessibility(self, plugin_override=True):
        ...
        vals = plugin_manager.call_hook(
            "course_accessibility", course=self, default=self._accessible
        )
    ...
    def get_tasks(self) -> dict[str, Task]:
        ...
        for task in tasks:
            try:
                ...
                logging.getLogger("inginious.course." + self._id).info(
                    "Couldn't load task %s : %s", task, str(e)
                )
        return output
    ...
    def lti_tool(self) -> ToolConfDict:
        ...
        for iss in self._lti_config:
            for client_config in self._lti_config[iss]:
                lti_tool.set_private_key(
                    iss,
                    client_config["private_key"],
                    client_id=client_config["client_id"],
                )
                lti_tool.set_public_key(
                    iss,
    ...
    def is_user_accepted_by_access_control(self, user_info: UserInfo):
        ...
        if (
            not user_info
            or self.get_access_control_method() not in keys_per_access_control_method
        ):
            return False
    ...
    def allow_unregister(self, plugin_override=True):
        ...
        vals = plugin_manager.call_hook(
            "course_allow_unregister", course=self, default=self._allow_unregister
        )
    ...
    def get_description(self, language):
        ...
        description = (
            self.gettext(language, self._description) if self._description else ""
        )
    ...
    def save(self):
        ...
        if self._new_doc:
            logging.getLogger("inginious.course").info(
                "Course %s created in the factory.", self._fs.prefix
            )
    ...
    def get(cls, courseid: str) -> Course:
        ...
        course = fetch_or_cache(
            course_fs, "course.yaml", lambda: _load_course(course_fs, courseid)
        )
        ...
        if i18n_fs.exists():
            for f in i18n_fs.list(folders=False, files=True, recursive=False):
                ...
                if ext == ".mo":
                    translations[lang] = fetch_or_cache(
                        i18n_fs, f, lambda: gettext.GNUTranslations(i18n_fs.get_fd(f))
                    )
    ...
    def delete(self):
        ...
        self._fs.delete()
        logging.getLogger("inginious.course").info(
            "Course %s erased from the factory.", self._fs.prefix
        )
    ...
    def get_all(cls) -> dict[str, Course]:
        ...
        for courseid in [
            f[0 : len(f) - 1]
            for f in get_fs_provider().list(folders=True, files=False, recursive=False)
        ]:
            try:
                output[courseid] = Course.get(courseid)
            except Exception as e:
                logging.getLogger("inginious.course").warning(
                    "Cannot open course : %s", courseid
                )
        return output
