...
def init(plugin_manager, client, config):
    ...
    class ExternalGrader(INGIniousPage):
        ...
        def keep_only_config_return_values(self, job_return):
            ...
            return {
                key: value
                for key, value in job_return.items()
        ...
        def POST(self):
            ...
            if "input" in post_input and "taskid" in post_input:
                ...
                try:
                    ...
                    response.response = [
                        json.dumps(
                            {"status": "error", "status_message": "Cannot decode input"}
                        )
                    ]
                    return response
                ...
                try:
                    ...
                    response.response = [
                        json.dumps(
                            {"status": "error", "status_message": "Cannot open task"}
                        )
                    ]
                    return response
                ...
                if not task.input_is_consistent(
                    ...
                    response.response = [
                        json.dumps(
                            {
                                "status": "error",
                                "status_message": "Input is not consistent with the task",
                            }
                        )
                    ]
                    return response
                ...
                if post_input.get("async") is None:
                    ...
                    try:
                        (
                            ...
                            0, course, task, task_input, "Plugin - Simple Grader"
                        )
                        job_return = {
                            "result": result,
                            "grade": grade,
                    ...
                    response.response = [
                        json.dumps(
                            dict(
                                list({"status": "done"}.items())
                                + list(
                                    self.keep_only_config_return_values(
                                        job_return
                                    ).items()
                                )
                            )
                    ...
                    # New async job
                    jobid = client_buffer.new_job(
                        course, task, task_input, "Plugin - Simple Grader"
                    )
                    response.response = [
                        json.dumps({"status": "done", "jobid": str(jobid)})
                    ]
                    return response
                ...
                if client_buffer.is_waiting(post_input["jobid"]):
                    ...
                    (
                        ...
                        stdout,
                        stderr,
                    ) = client_buffer.get_result(post_input["jobid"])
                    job_return = {
                        "result": result,
                        "grade": grade,
                        "problems": problems,
                    ...
                    response.response = [
                        json.dumps(
                            {
                                "status": "error",
                                "status_message": "There is no job with jobid {}".format(
                                    post_input["jobid"]
                                ),
                            }
                        )
                ...
                response.response = [
                    json.dumps(
                        {"status": "error", "status_message": "Unknown request type"}
                    )
                ]
                return response
