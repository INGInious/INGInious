...
class SubmissionGitSaver(threading.Thread):
    ...
    def __init__(self, plugin_manager, config):
        ...
        self.git = git.bake(
            "--work-tree=" + self.repopath,
            "--git-dir=" + os.path.join(self.repopath, ".git"),
        )
        if mustdoinit:
            self.git.init()
    ...
    def run(self):
        while True:
            try:
                submission, result, grade, problems, tests, custom, archive = (
                    self.queue.get()
                )
                self.save(submission, result, grade, problems, tests, custom, archive)
            except Exception as inst:
                self._logger.exception(
                    "Exception in JobSaver: " + str(inst), exc_info=True
                )
    ...
    def save(self, submission, result, grade, problems, tests, custom, archive):  # pylint: disable=unused-argument
        ...
        if not os.path.exists(
            os.path.join(self.repopath, submission["courseid"], submission["taskid"])
        ):
            os.mkdir(
                os.path.join(
                    self.repopath, submission["courseid"], submission["taskid"]
                )
            )
        ...
        open(os.path.join(dirname, "submitted_on"), "w+").write(
            str(submission["submitted_on"])
        )
        open(os.path.join(dirname, "input.yaml"), "w+").write(
            inginious.common.custom_yaml.dump(submission["input"])
        )
        result_obj = {"result": result[0], "text": result[1], "problems": problems}
        open(os.path.join(dirname, "result.yaml"), "w+").write(
            inginious.common.custom_yaml.dump(result_obj)
        )
        ...
        title = " - ".join(
            [
                str(submission["courseid"]) + "/" + str(submission["taskid"]),
                str(submission["_id"]),
                str.join("-", submission["username"]),
