...
class PluginManager(object):
    ...
    def _exception_free_callback(self, callback, *args, **kwargs):
        ...
        try:
            ...
            self._logger.exception(
                "An exception occurred while calling a hook! ", exc_info=True
            )
            return None
    ...
    def add_hook(self, name, callback, prio=0):
        ...
        add = (
            (
                lambda *args, **kwargs: self._exception_free_callback(
    ...
    def call_hook(self, name, **kwargs):
        ...
        return [
            y
            for y in [x(**kwargs) for x, _ in self._hooks.get(name, [])]
            if y is not None
        ]
