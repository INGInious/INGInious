...
class UpComingTasksBoard(INGIniousAuthPage):
    ...
    def page(self, time_planner):
        ...
        all_accessibilities = {}
        all_tasks = {
            courseid: course.get_tasks() for courseid, course in open_courses.items()
        }

        for courseid, course in open_courses.items():
            tasks = all_tasks[courseid]
            accessibilities = course.get_task_dispenser().get_accessibilities(
                tasks.keys(), [username]
            )[username]
            accessibilities = OrderedDict(
                sorted(accessibilities.items(), key=lambda x: x[1].get_soft_end_date())
            )
            all_accessibilities[courseid] = accessibilities

            outdated_tasks = [
                taskid
                for taskid, accessibility in accessibilities.items()
                if (not accessibility.is_open())
                or (
                    (accessibility.get_soft_end_date())
                    > (datetime.now() + timedelta(days=time_planner))
                )
            ]
            new_user_task_list = [
                taskid
                for taskid, accessibility in accessibilities.items()
                if accessibility.after_start() and taskid not in outdated_tasks
            ]

            tasks_data[courseid] = {
                taskid: {"succeeded": False, "grade": 0.0}
                for taskid in new_user_task_list
            }
            user_tasks = UserTask.objects(
                username=username,
                courseid=course.get_id(),
                taskid__in=new_user_task_list,
            )
            for user_task in user_tasks:
                if not user_task["succeeded"]:
                    tasks_data[courseid][user_task["taskid"]]["succeeded"] = user_task[
                        "succeeded"
                    ]
                    tasks_data[courseid][user_task["taskid"]]["grade"] = user_task[
                        "grade"
                    ]
        ...
        sorted_tasks = {
            courseid: OrderedDict(
                [
                    (taskid, all_tasks[courseid][taskid])
                    for taskid, accessibility in all_accessibilities[courseid].items()
                    if taskid in tasks_data[courseid]
                ]
            )
        ...
        open_courses = OrderedDict(
            sorted(
                list(open_courses.items()),
                key=lambda x: all_accessibilities[x[0]][
                    list(sorted_tasks[x[0]].keys())[0]
                ].get_soft_end_date(),
            )
        )

        return render_template(
            "upcoming_tasks/coming_tasks.html",
...
def init(plugin_manager, client, config):
    ...
    plugin_manager.add_page(
        "/coming_tasks", UpComingTasksBoard.as_view("upcomingtasksboardpage")
    )
    plugin_manager.add_page(
        "/plugins/coming_tasks/static/<path:path>",
        StaticMockPage.as_view("upcomingtasksstaticmockpage"),
    )
