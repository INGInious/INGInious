...
class HiddenUntilDirective(Directive, object):
    ...
    def run(self):
        ...
        try:
            ...
            raise self.error(
                'Unknown date format in the "%s" directive; '
        ...
        if after_deadline or force_show:
            ...
            if not after_deadline and force_show:
                ...
                self.add_name(node)
                text = _(
                    "The feedback below will be hidden to the students until :time:`{}`."
                ).format(hidden_until.isoformat())
    ...
    class _CustomHTMLTranslator(html4css1.HTMLTranslator, object):  # pylint: disable=abstract-method
        ...
        def starttag(self, node, tagname, suffix="\n", empty=False, **attributes):
            ...
            if (
                ...
                and not attributes["href"].startswith("#")
            ):
            ...
            return html4css1.HTMLTranslator.starttag(
        ...
        def visit_table(self, node):
            ...
            self.body.append(self.starttag(node, "table", CLASS=" ".join(classes)))
        ...
        def visit_admonition(self, node):
            ...
            cls = [
                x if not x.startswith("admonition-") else x[11:]
            ...
            if "title" in node and node["title"] != "":
                self.body.append(
                    self.starttag(node, "div", CLASS="card mb-3 border-" + cls)
                )
                ...
                self.body.append(
                    self.starttag(node, "div", CLASS="card-header " + card_color)
                )
...
class ParsableText(object):
        ...
        if self._parsed is None:
            try:
                if self._mode == "html":
                    ...
                    self._parsed = self.rst(
                        self._content, self._show_everything, debug=debug
                    )
            except Exception as e:
                if debug:
                    raise BaseException("Parsing failed") from e
                else:
                    self._parsed = _("<b>Parsing failed</b>: <pre>{}</pre>").format(
    ...
    def rst(cls, string, show_everything=False, initial_header_level=3, debug=False):
        ...
        parts = core.publish_parts(
            source=string, writer=_CustomHTMLWriter(), settings_overrides=overrides
        )
