...
class WebAppSubmissionManager:
    ...
    def add_job(self, course, task, inputdata, task_dispenser, debug=False, username=None):
        """
    ...
    def get_feedback_from_submission(self, submission, show_everything=False):
        """
            Get the input of a submission. If only_input is False, returns the full submissions with a dictionnary object at the key "input".
            Else, returns only the dictionnary.
        ...
        for problem in submission.problems:
            if isinstance(submission.problems[problem], str):  # fallback for old-style submissions
                submission.problems[problem] = (
                    submission.result,
                    ParsableText(submission["problems"][problem], "rst", show_everything).parse()
                )
            else:  # new-style submission
                try:
                    submission.problems[problem] = (
                        submission.problems[problem][0],
                        ParsableText(submission.problems[problem][1],"rst", show_everything).parse()
                    )
                except TypeError:
                    self._logger.error(
                    ...
                    submission.problems[problem] = (
                        'crash',
                        ParsableText(_("Feedback is badly formatted."),"rst", show_everything).parse()
                    )
        return submission
    ...
    def user_is_submission_owner(self, submission, username=None):
    ...
    def get_user_submissions(self, course, task, username=None):
