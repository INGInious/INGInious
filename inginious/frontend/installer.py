...
class Installer:
    ...
    def _ask_with_default(self, question, default=""):
        ...
        if self._default:
            ...
            answer = input(
                DOC + UNDERLINE + question + " [" + default + "]:" + ENDC + " "
            )
            if answer == "":
                answer = default
        return answer
    ...
    def _configure_directory(self, dirtype: str):
        ...
        self._display_question(
            "Please choose a directory in which to store the %s files." % dirtype
        )
        directory = None
        while directory is None:
            directory = self._ask_with_default(
                "%s directory" % (dirtype[0].upper() + dirtype[1:]), "./%s" % dirtype
            )
            if not os.path.exists(directory):
                if self._ask_boolean("Path does not exist. Create directory?", True):
                    try:
                        ...
                        self._display_error(
                            "Permission denied. Are you sure of your path?\nIf yes, contact your system administrator"
    ...
    def run(self):
        ...
        self.try_mongodb_opts(
            options["mongo_opt"]["host"], options["mongo_opt"]["database"]
        )
        ...
        while True:
            ...
            if start_port != "":
                try:
                    ...
                    self._display_error("Invalid number")
                    continue
                end_port = self._ask_with_default(
                    "End of the range", str(start_port + 100)
                )
                try:
        ...
        try:
            ...
            if (
                docker.utils.compare_version(
                    "1.24", docker_connection.version()["ApiVersion"]
                )
                < 0
            ):
    ...
    def ask_backend(self):
        ...
        if response:
            self._display_info(
                "If you use docker-machine on macOS, please see "
    ...
    def try_mongodb_opts(self, host="localhost", database_name="INGInious"):
        ...
        try:
            ...
            self._display_info(
                "Found mongodb server running version %s on %s." % (mongo_version, host)
            )
        except Exception as e:
            self._display_warning(
                "Cannot connect to MongoDB on host %s: %s" % (host, str(e))
            )
            return None
        ...
        try:
            ...
            self._display_warning(
                "Cannot access database %s: %s" % (database_name, str(e))
            )
            return None
        ...
        try:
            ...
            self._display_warning(
                "Cannot access gridfs %s: %s" % (database_name, str(e))
            )
            return None
    ...
    def configure_mongodb(self):
        ...
        if self.try_mongodb_opts(host, database_name) is not None:
            should_ask = self._ask_boolean(
                "Successfully connected to MongoDB. Do you want to edit the configuration anyway?",
                False,
            )
        else:
        ...
        while should_ask:
            ...
            self._display_question(
                "mongodb://USERNAME:PASSWORD@HOST:PORT/AUTHENTIFICATION_DATABASE"
            )
            ...
            if not self.try_mongodb_opts(host, database_name):
                if self._ask_boolean(
                    "Cannot connect to MongoDB. Would you like to continue anyway?",
                    False,
                ):
                    break
            else:
    ...
    def configure_task_directory(self):
        ...
        if os.path.exists(task_directory):
            self._display_question(
                "Demonstration tasks can be downloaded to let you discover INGInious."
            )
            if self._ask_boolean("Would you like to download them ?", True):
                try:
                    self._retrieve_and_extract_tarball(
                        "https://api.github.com/repos/INGInious/demo-tasks/tarball",
                        task_directory,
                    )
                    self._display_info(
                        "Successfully downloaded and copied demonstration tasks."
                    )
                except Exception as e:
                    self._display_error(
                        "An error occurred while copying the directory: %s" % str(e)
                    )
        else:
            self._display_warning(
                "Skipping copying the 'test' course because the task dir does not exists"
            )
    ...
    def select_containers_to_build(self):
        ...
        if not self._ask_boolean(
            "Build the default containers? This is highly recommended, and is required to build other containers.",
            True,
        ):
        ...
        try:
            ...
            for image in docker_connection.images.list():
                for tag in image.attrs["RepoTags"]:
                    if re.match(
                        r"^ghcr.io/inginious/env-(base|default):v" + __version__, tag
                    ):
        ...
        if len(stock_images) >= 2:
            self._display_info(
                "You already have the minimum required images for version "
                + __version__
            )
            if not self._ask_boolean("Do you want to re-build them ?", "yes"):
                self._display_info(
                    "Continuing with previous images. If you face issues, run inginious-container-update"
                )
                return
        try:
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                if "dev" in __version__:
                    tarball_url = (
                        "https://api.github.com/repos/INGInious/INGInious/tarball"
                    )
                    containers_version = "dev (github branch master)"
                    dev = True
                else:
                    tarball_url = (
                        "https://api.github.com/repos/INGInious/INGInious/tarball/v"
                        + __version__
                    )
                    containers_version = __version__
                    dev = False
                self._display_info(
                    "Downloading containers for version:" + containers_version
                )
                self._retrieve_and_extract_tarball(tarball_url, tmpdirname)
                self._build_container(
                    "ghcr.io/inginious/env-base",
                    os.path.join(tmpdirname, "base-containers", "base"),
                )
                self._build_container(
                    "ghcr.io/inginious/env-default",
                    os.path.join(tmpdirname, "base-containers", "default"),
                )
                if dev:
                    self._display_info(
                        "If you modified files in base-containers folder, don't forget to rebuild manually to make these changes effective !"
                    )

            # Other non-mandatory containers:
            with tempfile.TemporaryDirectory() as tmpdirname:
                self._display_info(
                    "Downloading the other containers source directory..."
                )
                self._retrieve_and_extract_tarball(
                    "https://api.github.com/repos/INGInious/containers/tarball",
                    tmpdirname,
                )
                ...
                available_containers = set(
                    os.listdir(os.path.join(tmpdirname, "grading"))
                )
                ...
                def __add_container(container):
                    ...
                    line_from = [
                        ...
                        for l in open(
                            os.path.join(
                                tmpdirname, "grading", container[22:], "Dockerfile"
                            )
                        )
                        .read()
                        .split("\n")
                    ...
                    if (
                        supercontainer.startswith("ingi/")
                        and supercontainer not in todo
                    ):
                        self._display_info(
                            "Container {} requires container {}, I'll build it too.".format(
            ...
            self._display_error(
                "An error occurred while copying the directory: {}".format(e)
            )
    ...
    def ldap_plugin(self):
        ...
        name = self._ask_with_default(
            "Authentication method name (will be displayed on the login page)", "LDAP"
        )
        prefix = self._ask_with_default(
            "Prefix to append to the username before db storage. Usefull when you have more than one auth method with "
            "common usernames.",
            "",
        )
        ...
        encryption = "none"
        while True:
            encryption = self._ask_with_default(
                "Encryption (either 'ssl', 'tls', or 'none')", "none"
            )
        ...
        request = self._ask_with_default(
            "Request to find a user. '{}' will be replaced by the username", "uid={}"
        )
        require_cert = self._ask_boolean(
            "Require certificate validation?", encryption is not None
        )
    ...
    def configure_authentication(self):
        ...
        username = self._ask_with_default(
            "Enter the login of the superadmin", "superadmin"
        )
        realname = self._ask_with_default(
            "Enter the name of the superadmin", "INGInious SuperAdmin"
        )
        email = None
        while not email:
            email = self._ask_with_default(
                "Enter the email address of the superadmin", "superadmin@inginious.org"
            )
            email = UserManager.sanitize_email(email)
            if email is None:
                self._display_error("Invalid email format.")

        password = self._ask_with_default(
            "Enter the password of the superadmin", "superadmin"
        )
        ...
        while True:
            if not self._ask_boolean(
                "Would you like to add another auth method?", False
            ):
                break

            self._display_info("You can choose an authentication plugin between:")
            self._display_info(
                "- 1. LDAP auth plugin. This plugin allows to connect to a distant LDAP host."
            )
            self._display_info(
                "There are other plugins available that are not configurable directly by inginious-install."
            )
            self._display_info(
                "Please consult the online documentation to install them yourself."
            )

            plugin = self._ask_with_default(
                "Enter the corresponding number to your choice", "skip"
            )
            if plugin == "1":
        ...
        options["session_parameters"]["timeout"] = self._ask_integer(
            "How much time should a user stay connected, "
            "in seconds? The default is 86400, one day.",
            86400,
        )
        options["session_parameters"]["secure"] = self._ask_boolean(
            "Do you plan to serve your INGInious instance only in HTTPS?", False
        )
        options["session_parameters"]["secret_key"] = hexlify(os.urandom(32)).decode(
            "utf-8"
        )
