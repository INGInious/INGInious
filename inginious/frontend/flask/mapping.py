...
from inginious.frontend.pages.admin.admin import (
...
from inginious.frontend.pages.lti.v1_1 import (
    LTI11LaunchPage,
    LTI11BindPage,
    LTI11LoginPage,
)
from inginious.frontend.pages.lti.v1_3 import (
    LTI13LaunchPage,
    LTI13BindPage,
    LTI13LoginPage,
    LTI13OIDCLoginPage,
...
def init_flask_maintenance_mapping(flask_app):
    flask_app.add_url_rule(
        "/", view_func=MaintenancePage.as_view("maintenancepage.alias")
    )
    flask_app.add_url_rule(
        "/<path:path>", view_func=MaintenancePage.as_view("maintenancepage")
    )
...
def init_flask_mapping(flask_app):
    ...
    flask_app.add_url_rule(
        "/register", view_func=RegistrationPage.as_view("registrationpage")
    )
    ...
    flask_app.add_url_rule(
        "/marketplace", view_func=MarketplacePage.as_view("marketplacepage")
    )
    flask_app.add_url_rule(
        "/marketplace/<courseid>",
        view_func=MarketplaceCoursePage.as_view("marketplacecoursepage"),
    )
    flask_app.add_url_rule(
        "/course/<courseid>", view_func=CoursePage.as_view("coursepage")
    )
    flask_app.add_url_rule(
        "/course/<courseid>/<taskid>", view_func=TaskPage.as_view("taskpage")
    )
    flask_app.add_url_rule(
        "/course/<courseid>/<taskid>/<path:path>",
        view_func=TaskPageStaticDownload.as_view("taskpagestaticdownload"),
    )
    flask_app.add_url_rule(
        "/group/<courseid>", view_func=GroupPage.as_view("grouppage")
    )
    flask_app.add_url_rule(
        "/auth/signin/<auth_id>",
    ...
    flask_app.add_url_rule(
        "/auth/callback/<auth_id>", view_func=CallbackPage.as_view("callbackpage")
    )
    flask_app.add_url_rule(
        "/pages/<pageid>", view_func=INGIniousStaticPage.as_view("staticpage")
    )
    flask_app.add_url_rule(
        "/courselist", view_func=CourseListPage.as_view("courselistpage")
    )
    flask_app.add_url_rule(
        "/mycourses", view_func=MyCoursesPage.as_view("mycoursespage")
    )
    flask_app.add_url_rule(
        "/preferences", view_func=PrefRedirectPage.as_view("prefredirectpage")
    )
    flask_app.add_url_rule(
        "/preferences/bindings", view_func=BindingsPage.as_view("bindingspage")
    )
    flask_app.add_url_rule(
        "/preferences/delete", view_func=DeletePage.as_view("deletepage")
    )
    flask_app.add_url_rule(
        "/preferences/profile", view_func=ProfilePage.as_view("profilepage")
    )
    flask_app.add_url_rule("/lti/task", view_func=LTITaskPage.as_view("ltitaskpage"))
    flask_app.add_url_rule(
        "/lti/<courseid>/<taskid>", view_func=LTI11LaunchPage.as_view("ltilaunchpage")
    )
    flask_app.add_url_rule("/lti/bind", view_func=LTI11BindPage.as_view("ltibindpage"))
    flask_app.add_url_rule(
        "/lti/login", view_func=LTI11LoginPage.as_view("ltiloginpage")
    )
    flask_app.add_url_rule(
        "/lti/asset/<path:asset_url>", view_func=LTIAssetPage.as_view("ltiassetpage")
    )

    flask_app.add_url_rule(
        "/lti1.3/oidc_login/<courseid>",
        view_func=LTI13OIDCLoginPage.as_view("lti1.3oidcloginpage"),
    )
    flask_app.add_url_rule(
        "/lti1.3/launch/<courseid>/<taskid>",
        view_func=LTI13LaunchPage.as_view("lti1.3launchpage"),
    )
    flask_app.add_url_rule(
        "/lti1.3/jwks/<courseid>/<keyset_hash>",
        view_func=LTI13JWKSPage.as_view("lti1.3jwkspage"),
    )
    flask_app.add_url_rule(
        "/lti1.3/bind", view_func=LTI13BindPage.as_view("lti1.3bindpage")
    )
    flask_app.add_url_rule(
        "/lti1.3/login", view_func=LTI13LoginPage.as_view("lti1.3loginpage")
    )

    flask_app.add_url_rule(
        "/admin/<courseid>", view_func=CourseRedirectPage.as_view("courseredirect")
    )
    flask_app.add_url_rule(
    ...
    flask_app.add_url_rule(
        "/api/v0/courses/<courseid>", view_func=APICourses.as_view("apicourses")
    )
    flask_app.add_url_rule(
        "/api/v0/courses/<courseid>/tasks",
        view_func=APITasks.as_view("apitasks.alias"),
        defaults={"taskid": None},
    )
    flask_app.add_url_rule(
        "/api/v0/courses/<courseid>/tasks/<taskid>",
        view_func=APITasks.as_view("apitasks"),
    )
    flask_app.add_url_rule(
        "/api/v0/courses/<courseid>/tasks/<taskid>/submissions",
        view_func=APISubmissions.as_view("apisubmissions.alias"),
    )
    flask_app.add_url_rule(
        "/api/v0/courses/<courseid>/tasks/<taskid>/submissions/<submissionid>",
