...
class RegistrationPage(INGIniousPage):
    ...
    def GET(self):
        ...
        if "activate" in data:
            ...
            msg = (
                _("Invalid activation hash.")
                if error
                else _("User successfully activated.")
            )
        elif "reset" in data:
    ...
    def get_reset_data(self, data):
        ...
        reset = None
        msg = ""
        user = User.objects(reset=data.get("reset", "")).first()
        if user is None:
            error = True
    ...
    def register_user(self, data):
        ...
        if not error:
            existing_user = User.objects(
                Q(username=data["username"]) | Q(email=email)
            ).first()
            if existing_user is not None:
                ...
                activate_hash = UserManager.hash_password_sha512(
...
""")
                    ...
                    message = Message(
                        recipients=[(data["realname"], email)],
                        subject=subject,
                        body=body,
                    )
                    mail.send(message)
    ...
    def lost_passwd(self, data):
        ...
        if not error:
            ...
            user = User.objects(email=data["recovery_email"]).modify(
                reset=reset_hash, new=True
            )
            if user is None:
                error = True
...
""").format(realname=user["realname"])
                    ...
                    message = Message(
                        recipients=[(user["realname"], data["recovery_email"])],
                        subject=subject,
                        body=body,
                    )
                    mail.send(message)
    ...
    def reset_passwd(self, data):
        ...
        if not error:
            passwd_hash = UserManager.hash_password(data["passwd"])
            user = User.objects(reset=data["reset"]).modify(
                set__password=passwd_hash,
                unset__reset=True,
                unset__activate=True,
                new=True,
            )
            if user is None:
                error = True
