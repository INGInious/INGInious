...
class CoursePage(INGIniousAuthPage):
    ...
    def show_page(self, course):
        ...
        if not self.user_manager.course_is_open_to_user(course, lti=False):
            ...
            last_submissions = []
            for submission in self.submission_manager.get_user_last_submissions(
                5, {"courseid": course.get_id(), "taskid__in": user_task_list}
            ):
            ...
            tasks_data = {
                taskid: {"succeeded": False, "grade": 0.0} for taskid in user_task_list
            }
            user_tasks = UserTask.objects(
                username=username, courseid=course.get_id(), taskid__in=user_task_list
            )
            ...
            course_grade = course.get_task_dispenser().get_course_grade(
                user_tasks, username
            )
