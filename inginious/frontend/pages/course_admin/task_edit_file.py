...
class CourseTaskFiles(INGIniousAdminPage):
    ...
    def GET_AUTH(self, courseid, taskid):  # pylint: disable=arguments-differ
        ...
        course, task = self.get_course_and_check_rights(
            courseid, taskid=taskid, allow_all_staff=False
        )
        task_fs = task.get_fs()

        user_input = request.args
        if (
            user_input.get("action") == "download"
            and user_input.get("path") is not None
        ):
            return self.action_download(task_fs, course, task, user_input.get("path"))
        elif (
            user_input.get("action") == "delete" and user_input.get("path") is not None
        ):
            return self.action_delete(task_fs, course, task, user_input.get("path"))
        elif (
            user_input.get("action") == "rename"
            and user_input.get("path") is not None
            and user_input.get("new_path") is not None
        ):
            ...
            user_input.get("action") == "create" and user_input.get("path") is not None
        ):
    ...
    def POST_AUTH(self, courseid, taskid):  # pylint: disable=arguments-differ
        ...
        if (
            user_input.get("action") == "upload"
            and user_input.get("path") is not None
            and user_input.get("file") is not None
        ):
            return self.action_upload(
                task_fs, course, task, user_input.get("path"), user_input.get("file")
            )
        elif (
            user_input.get("action") == "edit_save"
            and user_input.get("path") is not None
            and user_input.get("content") is not None
        ):
            return self.action_edit_save(
                task_fs, course, task, user_input.get("path"), user_input.get("content")
            )
        else:
            return self.show_tab_file(course, task)
    ...
    def show_tab_file(self, course, task, error=None):
        ...
        return render_template(
            "course_admin/edit_tabs/files.html",
            course=course,
            taskid=task.get_id(),
    ...
    def action_upload(self, task_fs, course, task, path, fileobj):
        ...
        try:
            ...
            return self.show_tab_file(
                course, task, _("An error occurred while writing the file")
            )
        return self.show_tab_file(course, task)
    ...
    def action_rename(self, task_fs, course, task, path, new_path):
        ...
        try:
            ...
            return self.show_tab_file(
                course, task, _("An error occurred while moving the files")
            )
    ...
    def action_delete(self, task_fs, course, task, path):
        ...
        try:
            ...
            return self.show_tab_file(
                course, task, _("An error occurred while deleting the files")
            )
