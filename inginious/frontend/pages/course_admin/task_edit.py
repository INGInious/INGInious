...
class CourseEditTask(INGIniousAdminPage):
    ...
    def GET_AUTH(self, courseid, taskid):  # pylint: disable=arguments-differ
        ...
        additional_tabs = plugin_manager.call_hook(
            "task_editor_tab", course=course, taskid=taskid, task_data=task_data
        )
    ...
    def parse_problem(self, problem_content):
        ...
        return (
            get_problem_types()
            .get(problem_content["type"])
            .parse_problem(problem_content)
        )
    ...
    def POST_AUTH(self, courseid, taskid):  # pylint: disable=arguments-differ
        ...
        try:
            ...
            environment_id = dict_from_prefix("environment_id", data).get(
                environment_type, ""
            )
            ...
            data["environment_id"] = (
                environment_id  # we do this after having removed all the environment_id[something] entries
            )
                ...
                data["problems"] = OrderedDict(
                    [
                        ...
                        for key, val in sorted(
                            iter(problems.items()), key=lambda x: int(x[1]["@order"])
                        )
                    ]
                )
            ...
            try:
                ...
                return json.dumps(
                    {
                        "status": "error",
                        "message": _("The number of random inputs must be an integer!"),
                    }
                )
            if data["input_random"] < 0:
                return json.dumps(
                    {
                        "status": "error",
                        "message": _("The number of random inputs must be positive!"),
                    }
                )
            ...
            return json.dumps(
                {
                    "status": "error",
                    "message": _("Your browser returned an invalid form ({})").format(
                        message
                    ),
                }
            )
        ...
        try:
            ...
            return json.dumps(
                {
                    "status": "error",
                    "message": _("Error while reading course's informations"),
                }
            )

        # Call plugins and return the first error
        plugin_results = plugin_manager.call_hook(
            "task_editor_submit", course=course, taskid=taskid, task_data=data
        )
        ...
        try:
            ...
            return json.dumps(
                {
                    "status": "error",
                    "message": _("Invalid data: {}").format(str(message)),
                }
            )
