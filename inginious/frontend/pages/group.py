...
class GroupPage(INGIniousAuthPage):
    ...
    def GET_AUTH(self, courseid):  # pylint: disable=arguments-differ
        ...
        if self.user_manager.has_staff_rights_on_course(course):
            raise Forbidden(
                description=_("You can't access this page as a member of the staff.")
            )
        elif not (
            self.user_manager.course_is_open_to_user(course, lti=False)
            ...
            if course.can_students_choose_group():
                ...
                new_group = Group.objects(
                    ...
                    __raw__={"$where": "this.students.length<this.size"},
                ...
                if new_group is None:
                    ...
                    self._logger.info(
                        "User %s registered to group %s/%s",
                        username,
                        courseid,
            ...
            if course.can_students_choose_group():
                ...
                if group is not None:
                    ...
                    group.save()
                    self._logger.info(
                        "User %s unregistered from group %s/%s",
                        username,
                        courseid,
                        group["description"],
                    )
        ...
        last_submissions = self.submission_manager.get_user_last_submissions(
            5, {"courseid": courseid, "taskid__in": list(tasks.keys())}
        )
        ...
        user_audiences = [
            audience.id
            for audience in Audience.objects(courseid=courseid, students=username)
        ]
        groups = self.user_manager.get_course_groups(course)

        student_allowed_in_group = lambda group: (
            any(set(user_audiences).intersection(group["audiences"]))
            or not group["audiences"]
        )
        allowed_groups = [group for group in groups if student_allowed_in_group(group)]

        users = self.user_manager.get_users_info(
