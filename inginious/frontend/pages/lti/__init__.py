    ...
    def POST_AUTH(self):
        ...
        if data:
            ...
            lti_user_profile = User.objects(**{
                "ltibindings__" + data["task"][0] + "__" + data[self._field]: data["username"]
            }).first()
            if not user_profile.ltibindings.get(data["task"][0], {}).get(data[self._field], "") and not lti_user_profile:
                # There is no binding yet, so bind LTI to this account
                user_profile.ltibindings.setdefault(data["task"][0], {})[data[self._field]] = data["username"]
                user_profile.save()
