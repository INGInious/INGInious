...
from inginious.frontend.flask.mapping import (
    init_flask_mapping,
    init_flask_maintenance_mapping,
)
...
def _put_configuration_defaults(config):
    ...
    if not session_parameters or "secret_key" not in config["session_parameters"]:
        print(
            "Please define a secret_key in the session_parameters part of the configuration.",
            file=sys.stderr,
        )
        print(
            "You can simply add the following (the text between the lines, without the lines) "
            "to your INGInious configuration file. We generated a random key for you.",
            file=sys.stderr,
        )
        print("-------------", file=sys.stderr)
        print("session_parameters:", file=sys.stderr)
        print(
            "\ttimeout: 86400  # 24 * 60 * 60, # 24 hours in seconds", file=sys.stderr
        )
        print(
        ...
        print(
            '\tsecret_key: "{}"'.format(hexlify(os.urandom(32)).decode("utf-8")),
            file=sys.stderr,
        )
        print("-------------", file=sys.stderr)
        exit(1)
    ...
    new_config = {
        ...
        # Session config
        "PERMANENT_SESSION_LIFETIME": session_parameters.get(
            "timeout", 86400
        ),  # 24 hours
        "SECRET_KEY": session_parameters["secret_key"],
        "SESSION_USE_SIGNER": True,
        "SESSION_COOKIE_NAME": session_parameters.get(
        ...
        new_config.update(
            {
                ...
                "MAIL_DEFAULT_SENDER": smtp_conf.get(
                    "sendername", "no-reply@ingnious.org"
                ),
            }
        )
    ...
    new_config["IS_TOS_DEFINED"] = (
        "PRIVACY_PAGE" in new_config and "TERMS_PAGE" in new_config
    )
...
def get_app(config):
    ...
    # Init database
    connect(
        config["MONGO_OPT"].get("database", "INGInious"),
        host=config["MONGO_OPT"].get("host", "localhost"),
        tz_aware=True,
    )

    # Fetch or init DB version
    db_version = (
        DBVersion.objects(db_version__exists=True).first() or DBVersion().save()
    )
    ...
    lti_score_publishers = {
        "1.1": LTIOutcomeManager(user_manager),
        "1.3": LTIGradeManager(user_manager),
    }

    submission_manager = WebAppSubmissionManager(
        client, user_manager, lti_score_publishers
    )
    ...
    flask_app.jinja_loader = jinja2.ChoiceLoader(
        [flask_app.jinja_loader, jinja2.PrefixLoader({})]
    )
    ...
    def add_lti_session_id(endpoint, values):
        if flask.session.is_lti:
            key = (
                "lti_session_id"
                if endpoint in ["ltibindpage", "lti1.3bindpage"]
                else "session_id"
            )
    ...
    # Loads plugins
    plugin_manager.load(
        client, flask_app, user_manager, submission_manager, config["PLUGINS"]
    )
